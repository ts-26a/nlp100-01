# -*- coding: utf-8 -*-
# from __future__ import unicode_literals

# This is python2

from random import sample
from re import findall
import sys
import codecs

sys.stdout = codecs.getwriter("utf-8")(sys.stdout)
# sys.stdin = codecs.getreader("utf-8")(sys.stdin)


class Nlp100_01(object):
    def q00(self):
        s = u"stressed"
        print s[::-1]

    def q01(self):
        s = u"パタトクカシー"
        print s[0::2]

    def q02(self):
        s1 = u"パトカー"
        s2 = u"タクシー"
        print ''.join(sum(zip(s1, s2), ()))

    def q03(self):
        s = u"Now I need a drink, alcoholic of course, after the heavy lectures involving quantum mechanics."
        s = map(len, findall(r"\w+", s))
        print s

    def q04(self):
        s = u"Hi He Lied Because Boron Could Not Oxidize Fluorine. New Nations Might Also Sign Peace Security Clause. Arthur King Can."
        arr = [1, 5, 6, 7, 8, 9, 15, 16, 19]
        s = s.split()
        ans = {}
        for x in range(1, len(s) + 1):
            if x in arr:
                ans[x] = s[x - 1][:1]
            else:
                ans[x] = s[x - 1][:2]
        print ans

    def q05_w_bi_gram(self, s, n):
        s = s.split()
        return [s[x:x + 2] for x in range(len(s) - 1)]

    def q05_l_bi_gram(self, s, n):
        return [s[x:x + 2] for x in range(len(s) - 1)]

    def q05(self):
        s = u"I am an NLPer"
        print self.q05_w_bi_gram(s, 2)
        print self.q05_l_bi_gram(s, 2)

    def q06_l_bi_gram(self, s, n):
        return [s[x:x + 2] for x in range(len(s) - 1)]

    def q06(self):
        s1 = u"paraparaparadise"
        s2 = u"paragraph"
        X = set(self.q06_l_bi_gram(s1, 2))
        Y = set(self.q06_l_bi_gram(s2, 2))
        print u"X = %s" %(X)
        print u"Y = %s" %(Y)
        print u"X | Y = %s" %(X | Y)
        print u"X & Y = %s" %(X & Y)
        print u"X - Y = %s" %(X - Y)
        print u"'se' in X = %s" %('se' in X)
        print u"'se' in Y = %s" %('se' in Y)

    def q07_tmp_new_str(self, x, y, z):
        return u"%s時の%sは%s" %(x, y, z)

    def q07(self):
        print self.q07_tmp_new_str(12, u"気温", 22.4)

    def q08_cipher(self, s):
        res = u""
        for x in s:
            if x.islower():
                res += chr(219 - ord(x))
            else:
                res += x
        return res

    def q08(self):
        print u"暗号化する文章を入力してしてください"
        s = raw_input(u"=>").decode('utf-8')
        c = self.q08_cipher(s)
        print u"暗号化: " + c
        print u"復号化: " + self.q08_cipher(c)

    def q09(self):
        print u"文章を入力してください"
        s = raw_input().decode('utf-8')
        ans = ""
        for x in s.split():
            if len(x) == 4:
                ans += x
            else:
                ans += x[0] + u"".join(sample(x[1:-1], len(x)-2)) + x[-1]
            ans += u" "
        print ans


nlp = Nlp100_01()
nlp.q00()
nlp.q01()
nlp.q02()
nlp.q03()
nlp.q04()
nlp.q05()
nlp.q06()
nlp.q07()
nlp.q08()
nlp.q09()
